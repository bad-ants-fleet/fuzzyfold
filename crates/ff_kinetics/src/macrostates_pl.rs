...
///
/// A `MacrostatePL` groups multiple concrete structures that
/// belong to the same basin or equivalence class in the folding landscape.
/// Here the macrostates are stored as PairLists, while the structures from 
/// which they are generated are given as DotBracketVectors.
...
///
/// # Notes
/// - The `MacrostateRegistyPL` initializes a "**catch-all** macrostate", which is
...
#[derive(Debug)]
pub struct MacrostatePL {
    name: String,
    ensemble: FxHashMap<PairList, (i32, f64)>,
...
impl MacrostatePL {
    ...
    pub fn from_list<E: EnergyModel>(
        ...
        for (_pl, (_en, prob)) in ensemble.iter_mut() {
            *prob /= q_sum;
        }
        Self {
            name: name.to_owned(),
    ...
    pub fn ensemble(&self) -> &FxHashMap<PairList, (i32, f64)> {
        &self.ensemble
    }
    ...
    pub fn get_lowest_microstate(&self) -> Option<&PairList> {
        self.ensemble
            .iter()
    ...
    pub fn get_random_microstate(&self) -> Option<PairList> {
...
pub struct MacrostateRegistryPL<E: EnergyModel> {
    sequence: NucleotideVec,
    energy_model: Arc<E>,
    /// By convention: macrostates[0] = unassigned.
    macrostates: Vec<MacrostatePL>,
}

impl<E: EnergyModel> From<(NucleotideVec, Arc<E>)> for MacrostateRegistryPL<E> {
    fn from((sequence, energy_model): (NucleotideVec, Arc<E>)) -> Self {
        let macrostates = vec![MacrostatePL::new_catch_all("Unassigned")];

        MacrostateRegistryPL {
            sequence,
            energy_model,
            macrostates,
        }
...
impl<E: EnergyModel> MacrostateRegistryPL<E> {
    ...
    pub fn insert_from_reader<R: BufRead>(&mut self, reader: R, source: &str
        ...
        self.macrostates.push(MacrostatePL::from_list(
            &name,
            &self.sequence,
            &structures,
            &*self.energy_model,
    ...
    pub fn classify(&self, structure: &DotBracketVec) -> usize {
        ...
        let pl = PairList::try_from(structure).unwrap();
    ...
    pub fn macrostates(&self) -> &Vec<MacrostatePL> {
        &self.macrostates
    }
    ...
    pub fn iter(&self) -> impl Iterator<Item = (usize, &MacrostatePL)> {
...
mod tests {
    ...
    fn test_macrostatepl_init() {
        ...
        */        
        ...
        let pl1 = PairList::try_from(&db1).unwrap();
        println!("Pl1 '{}':", pl1);

        let macrostate = MacrostatePL::from_list(
            "lmin=lm3_bh=3.0",
            &seq, 
        ...
        println!("Ensemble size: {}", macrostate.len());
        for (pl, (energy, prob)) in macrostate.ensemble() {
            println!("PairList: len = {}, pairs = {}, energy = {}, prob = {}", pl.len(), pl.len(), energy, prob);
        }
    ...
    fn test_macrostateplregistry_init_and_classify() {
        ...
        let mut registry = MacrostateRegistryPL::from((seq, Arc::new(energy_model)));
